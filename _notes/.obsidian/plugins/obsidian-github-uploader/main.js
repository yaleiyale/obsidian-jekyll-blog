...
var GHUploaderSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h3", {text: "GH Settings"});
    new import_obsidian.Setting(containerEl).setName("owner").setDesc("The account owner of the repository").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.owner).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("repo").setDesc("The name of the repository").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.repo).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("branch").setDesc("Target branch").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.branch).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("token").setDesc("Github token").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.token).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("message").setDesc("The commit message").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.message).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("name").setDesc("committer name").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.name).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("email").setDesc("committer email").addText((text) => {
      text.setPlaceholder("").setValue(this.plugin.settings.email).onChange(async (value) => {
        try {
    ...
    new import_obsidian.Setting(containerEl).setName("path").setDesc("target path").addText((text) => {
      text.setPlaceholder("image/ -> image/pic.png").setValue(this.plugin.settings.path).onChange(async (value) => {
        try {
...
var settings_tab_default = GHUploaderSettingTab;
...
var GHUploader = class extends import_obsidian2.Plugin {
  setupPasteHandler() {
    this.registerEvent(this.app.workspace.on("editor-paste", async (evt, editor) => {
      const {files} = evt.clipboardData;
      if (files.length > 0) {
        if (this.settings.owner && this.settings.repo && this.settings.branch && this.settings.token && this.settings.name && this.settings.email) {
          evt.preventDefault();
          for (const file of files) {
            ...
            this.getBase64(file).then((baseFile) => {
              const filePath = this.settings.path + this.getFileName() + file.name.substring(file.name.lastIndexOf("."));
              const jsonBody = {
              ...
              (0, import_axios.default)({
                url: `https://api.github.com/repos/${this.settings.owner}/${this.settings.repo}/contents/${filePath}`,
                method: "PUT",
                headers: {
                ...
                const MarkdownText = `![](https://fastly.jsdelivr.net/gh/${this.settings.owner}/${this.settings.repo}@${this.settings.branch}/${filePath})`;
  ...
  getFileName() {
    ...
    return time + (Math.random() * 10086).toString(36).slice(-6);
  }
