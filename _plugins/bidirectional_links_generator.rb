    ...
    all_docs.each do |current_note|
      all_docs.each do |note_potentially_linked_to|
        ...
        if note_potentially_linked_to.url.include?('.excalidraw')
          new_href = "#{site.baseurl}#{note_potentially_linked_to.url+'.svg'}"
        ...
        title_links.each do |title|
         current_note.content.gsub!(
          ...
          "<a class='internal-link' href='#{new_href}#{title.downcase}'>#{note_title_regexp_pattern}#{title}</a>"
         )
        end
      ...
      current_note.content = current_note.content.gsub(
        ...
        <<~HTML.delete("\n") # replace with this HTML (\\1 is what was inside the brackets)
          <span title='no link.' class='invalid-link'>
